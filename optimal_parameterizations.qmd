...
why the choice of parameterizations matters for MCMC methods,
and how to recognize situations where switching from one to the other may improve sampling efficiency.

There are many excellent presentations on the centered and non-centered parameterizations already.
The topic is discussed in the [Stan User's Guide](https://mc-stan.org/docs/stan-users-guide/) chapter "Efficiency Tuning"
...
develop and troubleshoot multi-level models for their data.


## Hierarchical models and MCMC samplers

A Bayesian model with $N$ parameters defines a probability density over an $N$-dimensional parameter space.
The diagram on the left (Figure 1 of Betancourt and Girolami)
...
and sampling efficiency depends on the geometric properties of this distribution.
The posterior distribution is the conditional distribution of the parameters given the data.
The interaction between the parameters and the data
determines the geometry of the posterior according to Bayes's theorem:
...
Every step requires computing the gradient of the posterior density at that point; this is computationally expensive.
...
The well-tuned HMC sampler requires the following:
...
- the step size is small enough to produce trajectories that obey Hamiltonian dynamics but no smaller.
- the mass matrix is a good linear preconditioner.
...
Appendix A of @Betancourt:2017 describes the algorithm in the current Stan NUTS-HMC implementation.
]

If the posterior is (approximately) multivariate normal (Gaussian),
then the mass matrix is a linear map that transforms it
into an isotropic Gaussian which is the easiest geometry for HMC.
...
When the posterior curvature (i.e., the Hessian) varies through the target density,
...
violate Hamiltonian dynamics, resulting in many divergences.
To avoid divergences, the step size is adjusted downwards.
The NUTS-HMC sampler sets an upper bound of $2^{\mathrm{max\_treedepth}} - 1$ on the number of steps.
...
This distribution exemplifies the difficulties of sampling from some hierarchical models.
It has support for $y \in \mathbb{R}$ and $x \in \mathbb{R}^9$ with density
$$
...
Below we show log-density contour plots for $x_1, y$ for each distribution.
...
and the statement `x = exp(y/2) * x_std` stretches a unit normal to scale `exp(y/2)`.
...
This ensures that each run starts from the same initial point.
]
We use the CmdStan defaults:  4 chains per run, each with 1,000 warmup and 1,000 sampling iterations.
...
properly converge during warmup and all other estimates should be ignored.^[$\hat R < 1.01$ is a guideline, not an absolute decree.]
Each fit returns 4,000 draws, but we must account for autocorrelated draws.
e[ESS](https://mc-stan.org/docs/reference-manual/analysis.html#effective-sample-size.section) is the *effective sample size*,
...
the individual parameter estimates of `y` and `x[1]` show poor $\hat R$ values, indicating a failure to converge;
the sample is not a representative sample from the target density.
The non-centered parameterization runs show that all chains converge.
...
We produce a 2D scatterplot between the population-level parameter
...
Red dots indicate divergent transitions, i.e., points where the sampler failed
...
As noted above, the R-hat diagnostics indicate that the centered parameterization has failed to converge,
...
at some point the contribution of the data distribution swamps that of the prior.
For this simple model, as the sampler sees more and more trials for a given group,
...
to these Stan programs to specify a simple hierarchical logistic regression model.
For a fixed number of groups (9) and a specified number of binary trials ('N'),
...
We follow the convention of using `y` for modeled data, `x` for unmodeled data,
and `N` for dimension size variable names, adding subscripts as needed.
For example, to generalize this from 9 groups to any number of groups,
we would define variables `N_groups`, and `N_obs`.
By convention, parameters are given Greek letter names, therefore
...
::: {.column-margin}
We use a fixed random seed for reproducibility; remove this to generate datasets with different values of `theta`.
:::
...
We to do this 100 times with different random seeds
...
the region of the true posterior density known as the "typical set" (see @Betancourt:2017 and
...
Running the sampler 100 times with different random seeds, for a total of 400 chains with different random initializations,
will reliably detect this.

The diagnostics $\hat R$ and bulk ESS are closely related: once the chains are sampling from the stationary distribution,
decreasing $\hat R$ corresponds directly to increasing ESS.
We therefore begin with $\hat R$ as a convergence screen.
...
because dividing by runtime introduces information that is not contained in $\hat R$.
Each point in the following plot is one run.
Runs where any of the parameters have $\hat R  > 1.01$ are shown at half opacity.
...
This is a very simple model which is perfectly calibrated to the data.
There is no reason to expect that this particular size range will be valid for other models and data.

Finally, we compare the computational cost directly to see
how the posterior geometry affects the amount of computation required by the sampler.
...
and `log_sigma_sq_std` and `theta_std[1]` for the non-centered parameterization.
To visualize all 16 fits, we divide fits by data regime:
the low-data regime: $2, 4, 16, 64$ with observations per group,
and the high-data regime with $128, 256, 2048, 32768$ observations per group.
...
With more data, the sampler is able to obtain tighter estimates of both `theta` and `log_sigma_sq`.
The high-data regime plots use an x-axis range of $(-3, 3)$ and a y-axis range of $(-4, 2)$,
(compared with an x-axis range of $(-4, 8)$ and y-axis range of $(-12, 6)$ for the low-data regime plots).
...
The variation across groups informs the scale `log_sigma_sq`.

In the centered coordinates, the tightly estimated `theta[j]` values depend only weakly
on the remaining uncertainty in the population variance,
and the scatterplot looks like a vertical line.
Increasing the number of observations per group does not eliminate uncertainty about `log_sigma_sq`
because the number of groups remains fixed at $9$.
Instead, the uncertainty approaches the uncertainty associated with estimating a population
...
This is a consequence of the inverse funnel geometry; during warmup, in order to move through
the highly curved posterior, the sampler adjusts the step size lower and lower;
during sampling, the small step size means that the sampler moves very very slowly.
...
Both models produce roughly the same estimates for both parameters.
...
However, without satisfactory sampler diagnostics, the estimates cannot be trusted if $\hat R > 1.01$.
...
In this case study, the dataset consists of 45 observations of at-bats for 18 major-league baseball players in the first several weeks of the 1970 baseball season.
This model can be used to estimate the expected batting ability of a new MLB player via the population level parameters
as well as to estimating the individual-level abilities of the 18 players in the dataset via the local parameter.
...
* Place a prior on the player ability $\theta$, the probability of getting a hit in an at-bat
  + $\theta$ is a probability (range from $0$ to $1$)
...
This is a hierarchical model with the canonical dependence between the population-level parameters `phi` and `kappa`
and the local-level parameter `theta` - the chance of success for an single at-bat.
Using only 45 observations per player puts this dataset squarely in the low-data regime.
The NUTS-HMC sampler struggles to fit the model and the HMC diagnostics show that the model fails to converge properly.

The next section of the baseball batting ability case study shows how the logistic regression model
which estimates a player's log-odds chance of success for one at-bat can be used instead of the beta-binomial model.
This leads to an excellent and thorough discussion of the non-centered parameterization.

The standard non-centered parameterization takes a hierarchical prior from a location-scale family.
Given this prior, the individual-level parameters can be expressed as a transformation of a standardized random variable.
A normal prior on player log-odds has this property but the Beta prior on player chance of success does not.
...
the case study [Pooling with Hierarchical Models for Repeated Binary Trials](https://mc-stan.org/learn-stan/case-studies/pool-binary-trials.html)
which is mainly focussed on how much can be estimated and predicted from the first 45 at-bats for a collection of 18 MLB players.
The dataset also provides the season totals for hits and at-bats.
...
This dataset has 18 groups (players), with group sizes (at-bats) ranging from 115 to 631.
...
The centered parameterization model is in the file `stan/baseball_hierarchical_logit_cp.stan`.


```{.stan include="stan/baseball_hierarchical_logit_cp.stan"}
```

The corresponding non-centered version is in the file `stan/baseball_hierarchical_logit_ncp.stan`.
...
constraint which does this internally. See [this example](https://mc-stan.org/docs/reference-manual/types.html#affine-transform.section).
...
- almost all the runs of centered parameterization have divergences across multiple chains
- a small fraction of the runs of the non-centered parameterization have one chain which produces many divergences.


This pattern suggests that the few chains which are failing are doing so because of poor random initialization
that puts their starting points far away from the posterior.
...
However, it reduces the average number of divergences from 80 to 0.19 and removes the extreme $\hat R$ values.
...
We choose players with the fewest, an intermediate number of, and the most at-bats.
...
The player's log-odds ability `alpha` is distributed normally with location `mu` and scale `sigma`.
...
with a 90% credible interval from 257 to 283.
...
the population mean as well as the population standard deviation.
Another factor here is that the real-world data were not generated from the binomial logit model;
...
Examination of the posterior geometry shows that the centered parameterization still gives
...
## Reparameterizing autoregressive coefficients
