...
while the non-centered parameterization shows the opposite behavoir.
...
Thus we know that the population level parameter variance parameter is distributed $\mathrm{Normal}(0, 1)$
and thus `log_sigma_sq` has true value $0$.
The true values for vector `theta` have been recorded.  The true value for `theta[1]` is:
...
### Sampler behavoirs in low and high data regimes
...
The data-generating value of theta[1] is (1.05). As the number of observations per group increases,
