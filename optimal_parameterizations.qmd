...
the centered parameterization reliably converges for datasets with 256 observations per group or higher
while the non-centered parameterization shows the opposite behavior.
What is especially notable is the extremely low `ESS_bulk/s` values for the non-centered parameterization
...
To see this, we inspect the stepsize and number of leapfrog steps that sampler takes
during each iteration together with the bulk ESS/second.
...
formats = {
    column: "{:,.2f}"
    if column in {"Stepsize", "Average leapfrog steps"}
    else "{:,.0f}"
    for column in numeric_columns
}
(
...
Finally, we examine the estimates of `log_sigma_sq` and `theta[1]`, the parameters of interest from both models.
...
```{python}
ESTIMATE_N_BY_PARAMETERIZATION = {
    "centered": N_OBS,
    "non-centered": N_OBS,
}

parameter_estimate_plot = plot_parameter_estimate_variability(
    data_estimate_runs,
    n_obs_by_parameterization=ESTIMATE_N_BY_PARAMETERIZATION,
    true_values={
        "log_sigma_sq": 0.0,
...
parameter_estimate_plot
```

Each point is the median of the 100 run-specific posterior medians.
The thin interval is the average run-specific posterior q05--q95 interval and therefore represents
within-run posterior uncertainty.
The thick interval spans the q05--q95 range of the 100 run-specific posterior medians and therefore
shows variation in the estimate across repeated fits.
The two intervals are offset slightly on the x-axis so that the generally much narrower thick interval remains visible.
Opacity is determined by the proportion of runs that pass the R-hat criteria.
...
For practical purposes, however, we want to choose the parameterization which reliably produces good diagnostics,
i.e., $\hat R < 1.01$ and a high effective sample size for the parameters of interest as well as
a high ESS_bulk/s.
...
In the large data regime it is especially important to use the centered parameterization,
...
Next we compare the estimates of `alpha`, the player ability for all 18 players,
ordered by the number of at-bats in the season.
The thin interval represents average within-run posterior uncertainty,
the thick interval represents variation in the posterior median across repeated fits.
...
We examine the population-level parameters from both parameterizations.
...
For `mu` $\approx -0.992$ and `sigma` $\approx 0.11$, this is approximately 271-272,
...
For these data, both parameterizations produce reliable fits and, when the sampler diagnostics are satisfactory, nearly identical posterior estimates.
This model is slightly more complex than the previous binomial logit model in that it estimates the both the population mean as well as the
population sd.
The dataset consists of 18 local groups, with differing numbers of observation per group, although they are all on roughly the same scale.
For this dataset, the non-centered parameterization provides the more robust and efficient performance.
