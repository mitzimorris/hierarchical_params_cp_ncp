...
and at greater length in [Betancourt and Girolami, 2013](https://arxiv.org/abs/1312.0906)
and [Papaspiliopoulos et al, 2007](https://arxiv.org/abs/0708.3797).
...
Here we show Figure 1 from Betancourt and Girolami which shows the structure of a simple hierarchical model
...
The canonical example of a challenging posterior density is [Neal's funnel](https://mc-stan.org/docs/stan-users-guide/efficiency-tuning.html#funnel.figure)
@Neal:2003 defines a distribution that exemplifies the difficulties of
sampling from some hierarchical models.
...
Following Betancourt and Girolami,  $y$ is the global parameter $\phi$,
and $x$ is local parameter $\theta$ and can be implemented directly in Stan as follows.
...
The Stan User's Guide presents Neal's funnel:
- show plot
Rename variables to make it clear what the parameters are.

Show the two parameterizations.
...
## To Center or to Non-Center

The Stan User's Guide and key Stan case studies (which have been translated into other PPL frameworks), provide small example datasets for which the non-centered parameterization is more efficient.
However, this is not always the case.   To illustrate, we write the data-generating model corresponding to hierarchical structure of the Neal's funnel model and generate several datasets of increasing size.
...
This case study expands on discussions in the [Stan User's Guide](https://mc-stan.org/docs/stan-users-guide/) 

## Hierarchical Models:  hierarhcical and group-level variance.

## MCMC Samplers: navigating high-dimensional posterior geometries.
