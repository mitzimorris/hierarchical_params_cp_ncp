...
To organize this work, we create a two helper classes
...
For very small amounts of data - 2 or 4 observations per group - the centered parameterization
had divergences.  Note, however, there were fewer divergences than when we fit the prior-only model in the previous section,
and as the amount of data increased, there were no divergences.

Again, we plot the space the sampler moves in:
parameters `theta[1]` and `log_sigma_sq` for the centered parameterization and
and `theta_raw[1]` against `log_sigma_sq` for the non-centered parameterization.
